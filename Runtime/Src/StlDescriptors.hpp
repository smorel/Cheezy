		...
		static void serialize(void* data,const ISerializer* s){
			...
			if(s->isLoading()){
				...
				for(Uint i=0;i<count;++i){
					Type t;
					DescriptorSerializationExtension* serializeExt = valueDesc->extension->GetOrCreateExtension<DescriptorSerializationExtension>();
					serializeExt->serialize(&t,s);//Direct Value serialization
					v.push_back(t);
				}
			}
			else{
	...
	public:
		...
		static void serialize(void* data,const ISerializer* s){
			DescriptorSerializationExtension* keySerializeExt = keyDesc->extension->GetOrCreateExtension<DescriptorSerializationExtension>();
			DescriptorSerializationExtension* valueSerializeExt = valueDesc->extension->GetOrCreateExtension<DescriptorSerializationExtension>();
			...
			if(s->isLoading()){
				for(Uint i=0;i<count;++i){
					KeyType k;
					keySerializeExt->serialize(&k,s);//Direct Value serialization
					ValueType t;
					valueSerializeExt->serialize(&t,s);//Direct Value serialization
					v[k] = t;
				}
			}
			else{
	...
	protected:\
		static void _buildDescriptor(){\
			if(!descriptor){\
				...
				descriptor = new StlDescriptor<name>(#name,sizeof(name),0,ext,"");\
				DescriptorSerializationExtension* serializeExt = ext->GetOrCreateExtension<DescriptorSerializationExtension>();\
				serializeExt->registerSerialize("serialize",&StlDescriptor<name>::serialize);\
	...
	protected:\
		static void _buildDescriptor(){\
			if(!descriptor){\
				DescriptorExtension* ext = new DescriptorExtension();\
				descriptor = new StlDescriptor< InterestType >(#name,sizeof(InterestType),0,ext,TypeNameConcatenator<T>::get());\
				DescriptorSerializationExtension* serializeExt = ext->GetOrCreateExtension<DescriptorSerializationExtension>();\
				serializeExt->registerSerialize("serialize",&StlDescriptor< InterestType >::serialize);\
				ReflectionManager::getInstance()->registerDescriptor(descriptor);\
			}\
		}\
		static Descriptor* descriptor;\
	...
	protected:\
		static void _buildDescriptor(){\
			if(!descriptor){\
				DescriptorExtension* ext = new DescriptorExtension();\
				descriptor = new StlDescriptor< InterestType >(#name,sizeof(InterestType),0,ext,TypeNameConcatenator<type1,type2>::get());\
				DescriptorSerializationExtension* serializeExt = ext->GetOrCreateExtension<DescriptorSerializationExtension>();\
				serializeExt->registerSerialize("serialize",&StlDescriptor< InterestType >::serialize);\
				ReflectionManager::getInstance()->registerDescriptor(descriptor);\
			}\
