	...
	AbstractObject Attribute::get(void* owner) const{
		...
		if(!extension->getter){
			...
			extension->getter->execute(owner, ret);
	...
	void Attribute::set(void* owner, const AbstractObject& o) const{
		if(!extension->setter){
			...
			s.push_back(o);
			extension->setter->execute(owner,s);
		}
	}
	...
	void Attribute::remove(void* owner, const AbstractObject& key) const{
		...
		if(!extension->remover){
			...
			std::vector<AbstractObject> s;
			s.push_back(key);
			extension->remover->execute(owner,s);
		}
	}
