...
Boolean MethodProxyBase::isAccessor(){
	_safeUpdate();
	return innerData().method && innerData().method->getMethodType() == MethodBase::eAccessor;;
}

void MethodProxyBase::execute(std::vector<Variant>& returnValues) {
	if(isValid() && isAccessor()){
		const AccessorBase* a = static_cast<const AccessorBase*>(innerData().method);
...
void MethodProxyBase::execute(const std::vector<AbstractObject>& paramValues) {
	if(isValid() && !isAccessor()){
		const MutatorBase* a = static_cast<const MutatorBase*>(innerData().method);
